...
function run_code($input) {
    ...
    foreach($input as $phrase) {
        ...
        foreach($words as $key => $word) {
            for($i = $key+1 ; $i<$word_count ; $i++) {
                if ($word == $words[$i]) {
                    $ok_passphrase = false;
                    continue 2;
                } else if (strlen($word) == strlen($words[$i])) {
                    // strings have the same length and might have the same chars... checkit!
                    $chars_1 = str_split($word);
                    $chars_2 = str_split($words[$i]);
                    sort($chars_1);
                    sort($chars_2);
                    if ($chars_1 == $chars_2) {
                        $ok_passphrase = false;
    ...
    echo "valid passphrases: $count\n";

}

#$input = ['abcde fghij', 'abcde xyz ecdab', 'a ab abc abd abf abj', 'iiii oiii ooii oooi oooo', 'oiii ioii iioi iiio'];
